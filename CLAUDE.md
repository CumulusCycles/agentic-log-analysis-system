...
- **`DASHBOARD_LLM_DRY_RUN` defaults to `true`** (safe-by-default cost-safety for the Phase 7e LangGraph agent). `POST /api/chat` returns a deterministic canned response without contacting OpenAI when this is `true`. Operator opts INTO paid spend by setting `DASHBOARD_LLM_DRY_RUN=false` in `.env` (and restarting `log-dashboard`). Tests + CI inherit dry-run through the Settings default. Never flip the code default — the asymmetry is deliberate (forgotten env var → dry-run, never surprise spend). **PR 4c extends this asymmetry to the proactive background scan loop:** when `DASHBOARD_LLM_DRY_RUN=true`, the loop wakes on schedule, logs `proactive_scan_skipped reason=llm_dry_run`, and skips graph invocation — real scans require BOTH `DASHBOARD_LLM_DRY_RUN=false` AND `DASHBOARD_PROACTIVE_SCAN_ENABLED=true`. See ADR-015 and ADR-016.
