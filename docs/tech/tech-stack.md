...
| Dashboard (frontend unit) | Vitest 3 + jsdom + `@testing-library/react` + `@testing-library/user-event` + `@testing-library/jest-dom` | 95 tests across 17 test files — AiChat, BarChart, ErrorDetail, LogExplorer, LogExplorer search-mode, LogGenerator, LoginPage, LogsFilterBar, Overview, Overview empty-corpus, ProactiveFindings, Sparkline, StatusBadge, StatusCard, TimeWindowSelector, usePolling, VectorstoreStats; mocks `fetch` via `vi.stubGlobal` |
