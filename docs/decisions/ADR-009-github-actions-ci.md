...
8. Branch protection on `main` (require CI checks before merge) is **recommended but deferred to public release**. As of 2026-06-03 the repo is private on GitHub's free tier, which does not allow branch protection on private repos (requires GitHub Pro or a public repo). The user has chosen to defer until the repo is made public at project completion. Until then, CI is informational only — red checks are visible on PRs but do not block merge.

   **A workflow refactor is required before enabling protection.** The current `dorny/paths-filter` design causes app jobs to report `skipped` on PRs that don't touch their code — and a `skipped` status does NOT satisfy a required check. Once protection is on, docs-only PRs and single-app PRs would be blocked indefinitely waiting for required app jobs that intentionally never ran. Two fix patterns to choose from at public-release time:
   - **(a) Umbrella job (recommended).** Add a `ci-summary` job that `needs:` every per-app job, runs `if: always()`, and checks each `needs.<job>.result` — pass when every job is `success` or `skipped`, fail on `failure` or `cancelled`. Put only `ci-summary` in branch protection's required-checks list. Individual app jobs continue to come and go via paths-filter; the umbrella always reports.
   - **(b) Drop CI's path scoping.** Run all unit tests on every PR. Simpler workflow; slightly higher CI cost (small with cache). Local `/ship` scoping is unaffected.
