...
| `GET` | `/api/status` | Per-app status cards for Overview Dashboard. v1.1.2 adds `embeddings_state: "ready" \| "loading" \| "unreachable"` so the frontend can distinguish cold-deploy-still-pulling-models from genuine "Ollama URL wrong." Written by the lifespan probe; updated by the embeddings-promotion watchdog when models become available. | 7b ✅ |
...
| `POST` | `/api/logs/search` | Semantic search over Chroma — body: query + filter clauses; returns LogEntry[] + scores. v1.1.2 adds `partial_corpus: bool` (additive, defaults False) reflecting `app.state.backfill_complete` so the frontend can distinguish "no matches" from "still indexing" during cold-start. | 7d ✅ |
...
| Bounds (6): `LLM_MAX_TOOL_CALLS_PER_REQUEST=4`, `LLM_MAX_INPUT_TOKENS_PER_REQUEST=8000` (Phase 8: `len(text) // 4` heuristic; Phase 7 used `tiktoken o200k_base`), `LLM_MAX_MESSAGES_PER_SESSION=40`, `LLM_TIMEOUT_SECONDS=120` (wall-clock cap on every Ollama HTTP call; sized for M1 Max llama3.1:8b cold start — operators with larger models tune up; forwarded via `client_kwargs={"timeout": N}` per [[reference_langchain_ollama_timeout_kwarg]]), `CHROMA_TIMEOUT_SECONDS=60` (v1.1.2 — symmetric with the Ollama cap; forwarded into `chromadb.config.Settings(chroma_query_request_timeout_seconds=N, chroma_sysdb_request_timeout_seconds=N)` at `HttpClient` construction so a hung Chroma can't stall chat/search/error-detail/proactive scan/backfill/watcher), `EMBEDDINGS_PROMOTION_INTERVAL_SECONDS=30` (v1.1.2 Option A — watchdog re-probe cadence for the eager-dashboard cold-start path; the dashboard comes up in seconds and the watchdog flips `loading → ready` once ollama-init finishes the model pull). | Deterministic in-graph bounds; no advisory middleware. Post-Phase-8 rationale is performance / UX bounding (was cost bounding) |
| Credential redaction in NEW `credentials.py` — `sanitize_user_input` + `sanitize_log_raw` cover Authorization-header (any scheme — Bearer/Basic/AWS4/Hawk/Negotiate, JSON-quote-aware so adjacent fields survive)/X-API-Key/password/JWT-shape/DSN | Defence-in-depth at BOTH input (router + ingest_node) AND output (tool `raw` field) |
