...
| Bounds (5): `LLM_MAX_TOOL_CALLS_PER_REQUEST=4`, `LLM_MAX_INPUT_TOKENS_PER_REQUEST=8000` (Phase 8: `len(text) // 4` heuristic; Phase 7 used `tiktoken o200k_base`), `LLM_MAX_MESSAGES_PER_SESSION=40`, `LLM_TIMEOUT_SECONDS=120` (wall-clock cap on every Ollama HTTP call; sized for M1 Max llama3.1:8b cold start — operators with larger models tune up; forwarded via `client_kwargs={"timeout": N}` per [[reference_langchain_ollama_timeout_kwarg]]), `CHROMA_TIMEOUT_SECONDS=60` (v1.1.2 — symmetric with the Ollama cap; forwarded into `chromadb.config.Settings(chroma_query_request_timeout_seconds=N, chroma_sysdb_request_timeout_seconds=N)` at `HttpClient` construction so a hung Chroma can't stall chat/search/error-detail/proactive scan/backfill/watcher). | Deterministic in-graph bounds; no advisory middleware. Post-Phase-8 rationale is performance / UX bounding (was cost bounding) |
