...
| Every OpenAI Chroma upsert prints a per-app level-count summary table to stdout + emits a structured `embedding_complete` log event | Operator-visible — answers "what did I just pay OpenAI to embed?" without parsing JSON. Skipped on dry-run and on full-dedup batches (no embed = no table) |
