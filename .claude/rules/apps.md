...
- **React SPA outbound headers (all React apps that drive a logging-app backend — FNOL, CP, AP):** every SPA's `request()` fetch wrapper MUST set `X-Source: prod` so real UX traffic lands in the `prod` source bucket. Single seam — every outbound call sets it; no call site should bypass the wrapper. Mirrors the Agitator's `build_client` single-seam pattern. Per ADR-011 2026-06-07 amendment. The dashboard frontend is exempt (it talks only to the dashboard backend, which doesn't write to an ingested log volume).
