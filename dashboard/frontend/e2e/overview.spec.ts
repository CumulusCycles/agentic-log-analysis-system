...
test.describe("Overview screen (7c)", () => {
  test("renders the Overview heading and four app cards", async ({
    loggedInPage: page,
  }) => {
    await page.goto("/");
    await expect(
    ...
    await expect(
      cards.filter({ has: page.getByText("shared-data-api") }),
    ).toHaveCount(1);
    await expect(
      cards.filter({ has: page.getByText("agent-portal") }),
    ).toHaveCount(1);
  });
  ...
  test("clicking the shared-data-api card drills down to /logs?app=shared-data-api", async ({
    ...
    await page.goto("/");
    await page
      .getByTestId("status-card")
      .filter({ hasText: "shared-data-api" })
      .click();
