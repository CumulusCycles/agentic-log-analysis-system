...
import { getClaims, getPolicies, getProfile, login } from "../lib/api";

/**
 * Single-seam test: every outbound fetch from Customer Portal's SPA must
...
describe("fetch wrapper — X-Source: prod single seam", () => {
  ...
  it("getPolicies sends X-Source: prod", async () => {
    await getPolicies("bearer");
    expect(lastHeaders().get("X-Source")).toBe("prod");
  });
