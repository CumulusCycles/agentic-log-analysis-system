...
test.describe("Authentication", () => {
  test("successful login navigates to /claims and persists token", async ({ page }) => {
    await page.goto("/login");
    await page.getByRole("textbox", { name: /username/i }).fill(AGENT1.username);
    await page.getByLabel(/password/i).fill(AGENT1.password);
    await page.getByRole("button", { name: /sign in/i }).click();

    await expect(page).toHaveURL(/\/claims$/);
    const token = await page.evaluate(() => localStorage.getItem("agent_portal_token"));
